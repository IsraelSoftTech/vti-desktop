    ...
    const cols: TableColumn<ClassRow>[] = [
      {
        ...
        render: (c) => (
          <input
            type="checkbox"
            className="fee-mgmt-matrix-check"
      ...
      },
      {
        ...
        title: "Class",
        width: 140,
        render: (c) => <span className="fee-mgmt-cell-strong">{c.name}</span>,
      },
      ...activeHeads.map((head) => ({
        ...
        render: (c: ClassRow) => (
          <input
            ...
            onChange={(e) => setCellValue(c.id, head.id, e.target.value)}
            className="fee-mgmt-matrix-input"
            placeholder="0"
            inputMode="decimal"
            aria-label={`${head.name} for ${c.name}`}
          />
        ),
      })),
      {
        ...
        width: 100,
        render: (c) => (
          <span className="fee-mgmt-matrix-total">{formatMoney(rowTotal(c.id))}</span>
        ),
      },
    ];
    return cols;
  ...
  return (
    <div className="fee-mgmt-scroll">
      <div className="fee-mgmt-card">
        ...
        <div className="fee-mgmt-matrix-toolbar">
          <span className="fee-mgmt-matrix-toolbar__count">
            {selectedClassIds.length} of {classes?.length ?? 0} classes selected
          </span>
          <div className="fee-mgmt-class-picker__actions">
            <button type="button" className="fee-mgmt-class-picker__link" onClick={selectAllClasses}>
              Select all
            </button>
            <button type="button" className="fee-mgmt-class-picker__link" onClick={clearClasses}>
              Clear
            </button>
          </div>
        </div>
        ...
        {feesLoading ? (
          <p className="fee-mgmt-matrix-loading">Loading class fees…</p>
        ) : classes?.length ? (
          activeHeads.length ? (
            <>
              <div className="fee-mgmt-matrix-table">
                <div className="fee-mgmt-matrix-table__select-head">
                  <label className="fee-mgmt-matrix-select-all">
                    <input
                      type="checkbox"
                      checked={allSelected}
                      ref={(el) => {
                      ...
                      onChange={() => (allSelected ? clearClasses() : selectAllClasses())}
                    />
              ...
              <PrimaryButton title={saveLabel} loading={busy} onClick={handleSave} fullWidth />
            </>
          ) : (
            <div className="fee-mgmt-placeholder">
              <Icon name="list-outline" size={28} />
              <p>Create fee types first, then set amounts per class here.</p>
            </div>
          )
        ) : (
          <div className="fee-mgmt-placeholder">
            <Icon name="school-outline" size={28} />
            <p>No classes found. Add classes in Academics first.</p>
          </div>
        )}
      </div>
    </div>
