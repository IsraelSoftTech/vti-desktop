...
import { savePdfAndShare } from "./idCardPdf";
...
export async function downloadClassList(options: {
  ...
  const filename = `${safeFilename(options.className)}-class-list.pdf`;
  const path = await savePdfAndShare(doc, filename);
  if (await Sharing.isAvailableAsync()) {
    await Sharing.shareAsync(path, {
      mimeType: "application/pdf",
      dialogTitle: `${options.className} — class list`,
      UTI: "com.adobe.pdf",
    });
  }
  return path;
