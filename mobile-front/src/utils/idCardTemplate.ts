...
export async function photoToDataUrl(
  ...
  if (options?.studentId) {
    try {
      ...
      const dest = `${FileSystem.cacheDirectory}idcard_proxy_${options.studentId}.jpg`;
      const { uri, status } = await FileSystem.downloadAsync(
        `${attendanceApiRoot()}/students/${options.studentId}/photo`,
        dest,
        { headers }
      );
  ...
  try {
    const dest = `${FileSystem.cacheDirectory}idcard_${Date.now()}.jpg`;
...
export async function logoToDataUrl(path?: string | null): Promise<string> {
  try {
    ...
    const headers: Record<string, string> = {};
    if (token) headers.Authorization = `Bearer ${token}`;
    const dest = `${FileSystem.cacheDirectory}idcard_school_logo.bin`;
    const { uri, status } = await FileSystem.downloadAsync(
      `${attendanceApiRoot()}/settings/logo`,
      dest,
      { headers }
    );
    if (status >= 200 && status < 300) {
      const base64 = await FileSystem.readAsStringAsync(uri, { encoding: "base64" });
      if (base64) return `data:${mimeFromBase64(base64)};base64,${base64}`;
    }
  } catch {
  ...
  if (src.startsWith("data:")) {
    ...
    return b64 ? `data:${mimeFromBase64(b64)};base64,${b64}` : "";
  }
  try {
    const dest = `${FileSystem.cacheDirectory}idcard_school_logo_url.bin`;
    const { uri, status } = await FileSystem.downloadAsync(src, dest);
    if (status >= 200 && status < 300) {
      const base64 = await FileSystem.readAsStringAsync(uri, { encoding: "base64" });
      if (base64) return `data:${mimeFromBase64(base64)};base64,${base64}`;
    }
