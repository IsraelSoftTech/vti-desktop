...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  body { font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif; color: #333; padding: 28px; font-size: 11px; }
  .header { border-bottom: 3px solid #2d5bff; padding-bottom: 14px; margin-bottom: 18px; }
  .school { font-size: 18px; font-weight: 800; color: #1a53ff; }
  .title { font-size: 14px; font-weight: 700; margin-top: 4px; color: #555; }
  .student { background: #edf0ff; border-radius: 10px; padding: 14px; margin-bottom: 16px; display: flex; justify-content: space-between; gap: 12px; }
  .student-name { font-size: 16px; font-weight: 800; color: #1a53ff; }
  .student-meta { font-size: 11px; color: #666; margin-top: 4px; line-height: 1.5; }
  .summary { display: flex; gap: 10px; margin-bottom: 18px; }
  .summary-box { flex: 1; border: 1px solid #d8dff0; border-radius: 10px; padding: 12px; text-align: center; }
  .summary-val { font-size: 16px; font-weight: 800; color: #2d5bff; }
  .summary-label { font-size: 9px; color: #888; margin-top: 2px; text-transform: uppercase; letter-spacing: 0.4px; }
  ...
  h3 { font-size: 12px; font-weight: 800; color: #1a53ff; margin: 16px 0 8px; }
  table { width: 100%; border-collapse: collapse; margin-bottom: 8px; }
  th { background: #2d5bff; color: #fff; padding: 7px 8px; font-size: 9px; text-transform: uppercase; letter-spacing: 0.3px; }
  td { padding: 7px 8px; border-bottom: 1px solid #e8ecf8; }
  tr:nth-child(even) td { background: #f8f9ff; }
  .c { text-align: center; }
  .r { text-align: right; }
  .l { text-align: left; }
  .muted { color: #999; }
  .footer { margin-top: 20px; padding-top: 10px; border-top: 1px solid #d8dff0; display: flex; justify-content: space-between; color: #999; font-size: 9px; }
</style>
</head>
<body>
  <div class="header">
...
export async function printFeeRecord(record: FeeRecord, schoolName?: string) {
  ...
  await Sharing.shareAsync(uri, {
    ...
    dialogTitle: `Fee Statement — ${record.student.fullName}`,
    UTI: "com.adobe.pdf",
  });
  return uri;
}
